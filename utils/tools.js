 ...
 */
export const platformImg = (img = consts.IMG_ERROR, suffix = 'png') => {
  if (img.includes('http://') || img.includes('https://')) { return img }

  let imgUrl = ''

  // #ifdef H5
  imgUrl = process.env.NODE_ENV === 'development' ? `/static${img}.${suffix}` : `${location.origin}/phone/static${img}.${suffix}`
  // #endif

  // #ifndef H5
  imgUrl = process.env.NODE_ENV === 'development' ? `/static${img}.${suffix}` : `${consts.SERVER_PATH}/phone/static${img}.${suffix}`
  // #endif
